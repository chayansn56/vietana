  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="flex items-center justify-between p-4 sm:p-6 lg:px-12 bg-[#FAF8F3]/80 dark:bg-[#111615]/80 backdrop-blur-xl absolute top-0 left-0 right-0 z-[3010] border-b border-[#1D1D1F]/5 dark:border-white/10 shadow-sm">
            <div>
              <Text className="text-[10px] tracking-widest uppercase text-[#1E4D45] dark:text-brand-gold font-bold mb-1">
                Cartography
              </Text>
              <Heading as="h2" className="text-2xl font-serif text-[#1D1D1F] dark:text-white tracking-wider">
                Explore Vietnam
              </Heading>
            </div>
            <button 
              onClick={onClose}
              className="group flex items-center gap-3 px-4 py-2 rounded-full bg-white dark:bg-[#1A2120] hover:bg-[#F2EFE8] dark:hover:bg-[#1A2120]/80 transition-colors border border-[#1D1D1F]/10 dark:border-white/10 shadow-sm"
            >
              <Text className="text-xs tracking-[0.2em] uppercase text-[#1D1D1F]/70 dark:text-white/70 group-hover:text-[#1D1D1F] dark:group-hover:text-white font-bold hidden sm:block">
                Close Map
              </Text>
              <Icon name="X" size={18} className="text-[#1D1D1F] dark:text-white" />
            </button>
          </div>
