...
const FloatingHelpAndChat: React.FC<FloatingHelpAndChatProps> = ({ onPlannerClick, onWhatsAppClick }) => {
  ...
  return (
    <div className="fixed bottom-[92px] right-6 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8 z-[1010] flex flex-col items-end gap-3 pointer-events-none">
      ...
      {/* 2. Scroll Mode: Show Compact WhatsApp & Zalo Buttons stacked horizontally or vertically */}
      {!isAtHero && (
        <div className="flex flex-row md:flex-col gap-2 pointer-events-auto">
          {/* Zalo Button (Compact) */}
          <div 
            className="cursor-pointer flex items-center justify-center w-11 h-11 bg-[#0068FF] text-white rounded-full shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-300"
            onClick={handleZalo}
            title="Chat on Zalo"
          >
          ...
          <div 
            className="cursor-pointer flex items-center justify-center w-11 h-11 bg-[#25D366] text-white rounded-full shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-300"
            onClick={handleWhatsApp}
            title="Chat on WhatsApp"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor" className="opacity-90">
