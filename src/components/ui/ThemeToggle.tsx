...
const ThemeToggle: React.FC<ThemeToggleProps> = ({ className, isNavbar = false, isLight = false }) => {
  ...
  return (
    <button
      ...
      className={`focus-ring ${className || 'cursor-pointer flex items-center justify-center w-10 h-10 rounded-full border-none bg-gradient-to-br from-[#D4AF37] via-[#E8C84A] to-[#1E4D45] text-white shadow-[0_4px_12px_rgba(212,175,55,0.35)] hover:shadow-[0_4px_18px_rgba(212,175,55,0.6)] hover:scale-105 active:scale-95 transition-all duration-300 pointer-events-auto'}`}
    >
